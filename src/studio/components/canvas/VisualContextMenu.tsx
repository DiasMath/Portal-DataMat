...
export function VisualContextMenu({ visualId, x, y, onClose }: VisualContextMenuProps) {
  ...
  const handleRename = () => {
    setRenameValue(visual.title);
    setIsRenaming(true);
  };

  const handleRenameSubmit = () => {
    if (renameValue.trim()) {
      dispatch({ type: 'UPDATE_VISUAL', payload: { id: visualId, updates: { title: renameValue.trim() } } });
    }
    setIsRenaming(false);
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      {menuItems.map((item, i) => {
        ...
        const menuItem = item as { icon: React.ReactNode; label: string; shortcut?: string; action: () => void; danger?: boolean };
        return (
          <button
            key={i}
            onClick={() => { menuItem.action(); if (!isRenaming) onClose(); }}
