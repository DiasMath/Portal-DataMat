...
import { updateConnectionGroup, deleteConnectionGroup } from '@/lib/connections/groups-repository';
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const success = await updateConnectionGroup(currentUser.uid, id, updates);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const success = await deleteConnectionGroup(currentUser.uid, id);
  if (!success) return NextResponse.json({ error: 'Grupo não encontrado' }, { status: 404 });
  return NextResponse.json({ success: true });
}
